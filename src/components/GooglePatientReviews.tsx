...
export function GooglePatientReviews({ heading, reviews, label, description, language }: GooglePatientReviewsProps) {
  ...
  // Check if sliding is required based on screen size and number of active reviews
  const isSliderNeeded = activeReviews.length > visibleCards;

  const displayReviews = React.useMemo(() => {
    if (activeReviews.length === 0) return [];
    
    // Always prepend and append exactly 3 wrapped elements to maintain consistent index offset (currentIndex + 3)
    const prepended = [
      activeReviews[(activeReviews.length - 3 + activeReviews.length * 10) % activeReviews.length],
      activeReviews[(activeReviews.length - 2 + activeReviews.length * 10) % activeReviews.length],
      activeReviews[(activeReviews.length - 1 + activeReviews.length * 10) % activeReviews.length]
    ];
    
    const appended = [
      activeReviews[0 % activeReviews.length],
      activeReviews[1 % activeReviews.length],
      activeReviews[2 % activeReviews.length]
    ];
  ...
  const renderCardContent = (review: Review) => {
    return (
  ...
  return (
    <div className="space-y-6 sm:space-y-10 pt-6 sm:pt-14 border-t border-slate-200/60 animate-fade-in" id="google-patient-reviews-section">
      ...
      {/* Cards Slider / Static Grid Layout */}

...
import { Maximize2, X, ChevronLeft, ChevronRight, ImageIcon, Heart } from 'lucide-react';
import { SocialServiceItem } from '../types';
import { socialServiceService } from '../utils/socialServiceData';
import { useSEO } from '../utils/seo';

export default function SocialServiceGujarati() {
  useSEO({
    title: 'સામાજિક સેવા અને મફત ડેન્ટલ કેમ્પ | પટેલ ડેન્ટલ હોસ્પિટલ',
    description: 'પટેલ ડેન્ટલ હોસ્પિટલ, રાજકોટ દ્વારા ચલાવવામાં આવતા સામાજિક સેવા અને સમુદાય કલ્યાણ કાર્યક્રમો. મફત ડેન્ટલ ચેકઅપ અને આરોગ્ય જાગૃતિ કેમ્પ.',
    keywords: 'સામાજિક સેવા, મફત ડેન્ટલ કેમ્પ રાજકોટ, પટેલ ડેન્ટલ હોસ્પિટલ'
  });
  ...
  return (
    <div id="social-service-page-view" className="bg-[#FAFAFC] min-h-screen gujarati-text">
      
      {/* Hero Header Section */}
      <section className="pt-[108px] sm:pt-[124px] lg:pt-[140px] pb-12 bg-white border-b border-slate-100 relative">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto space-y-4">
            <span className="text-[#0D9488] font-extrabold text-xs tracking-widest uppercase flex items-center justify-center gap-1.5 mb-1 gujarati-text">
              <Heart className="h-4 w-4 fill-[#0D9488]/10 animate-pulse" /> સમુદાય સેવા • સામાજિક જવાબદારી
            </span>
            ...
            <p className="text-slate-500 text-sm sm:text-base font-sans leading-relaxed gujarati-text">
              Patel Dental Hospital વિવિધ આરોગ્ય જાગૃતિ કાર્યક્રમો, મફત ડેન્ટલ કેમ્પ, શૈક્ષણિક પ્રવૃત્તિઓ, સમુદાય સુધી પહોંચ અને જાહેર કલ્યાણની પહેલ દ્વારા સમાજની સેવા કરવા માટે પ્રતિબદ્ધ છે.
            </p>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="mt-4 text-xs font-bold text-slate-400 uppercase tracking-widest gujarati-text">ગેલેરી લોડ થઈ રહી છે...</p>
            </div>
          ) : items.length === 0 ? (
            <div className="bg-white rounded-[24px] p-12 text-center border border-slate-100 shadow-3xs max-w-md mx-auto">
              ...
              <p className="text-slate-700 font-bold text-sm gujarati-text">કોઈ સામાજિક સેવાના ફોટા હજુ ઉપલબ્ધ નથી.</p>
              <p className="text-slate-400 text-xs mt-1 gujarati-text">કૃપા કરીને પછીથી ફરી તપાસો અથવા એડમિન પેનલ દ્વારા ફોટા ગોઠવો.</p>
            </div>
          ) : (
                ...
                >
                  ...
                  {item.title && (
                    <div className="mt-3 px-1 pb-1 text-center">
                      <h3 className="font-display font-semibold text-slate-800 text-xs sm:text-sm line-clamp-2 group-hover:text-[#0D9488] transition-colors leading-snug gujarati-text">
                        {item.title}
                      </h3>
                    </div>
                  )}
              ...
              >
                ...
                {currentLightboxImg.title && (
                  <div className="p-4 bg-slate-900 border-t border-white/10 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <h3 className="font-display font-bold text-sm sm:text-base text-white gujarati-text">
                      {currentLightboxImg.title}
                    </h3>
                    {lightboxIndex !== null && (
